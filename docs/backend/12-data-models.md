...
5. **Role.id is auto-increment** — Using auto-increment integers for roles makes IDs environment-dependent and fragile for test data.
6. **`is_active` on OrganizationMember is redundant** — Soft delete (`deleted_at`) already handles deactivation. The `is_active` boolean creates ambiguity.
7. **BundlePlan uses paranoid mode unnecessarily** — As a pure junction table, soft deletes add complexity without clear benefit.
8. **`OneTimePurchase` lacks plan/bundle validator** — Unlike Subscription, which validates that either `plan_id` or `bundle_id` is set, `OneTimePurchase` has no such validation.
9. **`stripe_subscription_id` not unique** — The Subscription model doesn't enforce uniqueness on this field.
10. **Credentials stored as plaintext JSONB** — `IntegrationAccount` and `GlobalIntegration` store sensitive OAuth tokens without encryption.
...
15. **`system_config` values are untyped** — All config values are stored as TEXT with no schema or type validation.
