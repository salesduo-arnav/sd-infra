                            ...
                            <div className="space-y-2">
                                ...
                                <div className="grid gap-2">
                                    {INTEGRATION_TYPE_OPTIONS.map((opt) => {
                                        ...
                                        const SP_API_VALUES = ["sp_api", "sp_api_sc", "sp_api_vc"];
                                        const isSpApiOption = SP_API_VALUES.includes(opt.value);
                                        return (
                                            <button
                                                key={opt.value}
                                                type="button"
                                                onClick={() => {
                                                    setFormData(prev => {
                                                        if (isSelected) {
                                                            // Deselect
                                                            return { ...prev, required_integrations: prev.required_integrations.filter(v => v !== opt.value) };
                                                        }
                                                        // Select: if SP-API option, remove the other SP-API options (mutual exclusivity)
                                                        const filtered = isSpApiOption
                                                            ? prev.required_integrations.filter(v => !SP_API_VALUES.includes(v))
                                                            : prev.required_integrations;
                                                        return { ...prev, required_integrations: [...filtered, opt.value] };
                                                    });
                                                }}
                                            ...
                                            >
                                                ...
                                                <div
                                                    className={`h-4 w-4 rounded-full border flex items-center justify-center shrink-0 transition-colors ${isSelected
                                                ...
                                                {isSpApiOption && <span className="text-[10px] text-muted-foreground">SP-API</span>}
                                            </button>
                                        );
                                    })}
                                </div>
