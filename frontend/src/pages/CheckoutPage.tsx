...
export default function CheckoutPage() {
  ...
  const initializeCheckout = useCallback(async () => {
    ...
    try {
      ...
      if (response.data.clientSecret) {
        ...
        setError(t('pages.checkout.failedToInitialize'));
      }
    } catch (err: unknown) {
      console.error(err);
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      setError((err as any).response?.data?.message || t('pages.checkout.initializationError'));
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    if ((!planId && !bundleId && (!cartItems || cartItems.length === 0))) {
      setError(t('pages.checkout.noItemsSelected'));
      return;
    }
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gray-50/50">
        <Card className="w-[400px] border-destructive/20 shadow-lg">
          <CardHeader>
            <CardTitle className="text-destructive">{t('pages.checkout.configError')}</CardTitle>
            <CardDescription>{t('pages.checkout.stripeNotConfigured')}</CardDescription>
          </CardHeader>
          <CardContent>
            <Button variant="outline" className="w-full" onClick={() => navigate('/plans')}>{t('pages.checkout.returnToPlans')}</Button>
          </CardContent>
        </Card>
      </div>
    );
  ...
  if (error) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-gray-50/50">
        <Card className="w-[400px] border-destructive/20 shadow-lg">
          <CardHeader>
            <CardTitle className="text-destructive">{t('pages.checkout.checkoutError')}</CardTitle>
        ...
        <div className="flex flex-col gap-6 order-2 lg:order-1 h-full max-h-[calc(100vh-6rem)] overflow-hidden">
          ...
          <Card className="flex-1 flex flex-col border-none shadow-md bg-white overflow-hidden">
            <CardHeader className="bg-primary/5 pb-6 border-b">
              <CardTitle className="text-2xl">{t('pages.checkout.orderSummary')}</CardTitle>
              <CardDescription>{t('pages.checkout.reviewDetails')}</CardDescription>
            </CardHeader>
            ...
            <ScrollArea className="flex-1 w-full relative">
              <CardContent className="p-6 space-y-6">
                {cartItems ? (
                        ...
                        <div className="flex justify-between items-start">
                          ...
                          <p className="font-bold text-lg">
                            ...
                            {item.period === 'one_time' ? (
                              <span className="text-sm text-muted-foreground font-normal"> {item.currency || 'USD'} ({t('pages.checkout.oneTime')})</span>
                            ) : (
                  ...
                  <p className="text-muted-foreground">{t('pages.checkout.loadingDetails')}</p>
                )}
              </CardContent>
            </ScrollArea>
            ...
            <div className="p-6 bg-muted/10 border-t mt-auto">
              ...
              <div className="flex justify-between items-center mb-4">
                <span className="text-muted-foreground">{t('pages.checkout.tax')}</span>
                <span className="text-xs text-muted-foreground">{t('pages.checkout.taxCalculated')}</span>
              </div>
              <div className="flex justify-between items-center pt-4 border-t border-dashed">
                <span className="text-lg font-bold">{t('pages.checkout.totalDueToday')}</span>
              ...
              {clientSecret ? (
                ...
                <div className="flex h-full items-center justify-center min-h-[400px]">
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-muted-foreground text-sm">{t('pages.checkout.initializingCheckout')}</p>
                  </div>
                </div>
              )}
            </div>
