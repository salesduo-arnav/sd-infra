...
export default function Plans() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        toast.error(t('pages.plans.failedToLoadPlans'));
      } finally {
        setIsLoading(false);
        // Enable transitions
  ...
  const handleStartTrial = async (toolId: string) => {
    ...
    try {
      ...
      toast.success(t('pages.plans.trialStarted'));
      await fetchSubscriptions();
      // Refresh eligibility
  ...
  const handleCheckout = () => {
    ...
    if (trialVariations.size > 1) {
      toast.error(t('pages.plans.mixedTrialError'));
      return;
    }
    ...
    if (itemsToCheckout.some(i => i.interval !== firstInterval)) {
      toast.error(t('pages.plans.mixedIntervalError'));
      return;
    }
    ...
    if (cart.some(item => (item.currency || 'USD') !== firstCurrency)) {
      toast.error(t('pages.plans.mixedCurrencyError'));
      return;
    }
  ...
  const handleSubscriptionChange = async () => {
    ...
    try {
      ...
      if (hasUpgrade) {
        toast.success(t('pages.plans.upgradeSuccess'));
      } else if (hasDowngrade) {
        toast.success(t('pages.plans.downgradeScheduled'));
      }
      ...
      console.error(error);
      toast.error(t('pages.plans.updateFailed'));
    } finally {
        ...
        <div className="p-4 border-b bg-gradient-to-r from-primary/5 to-transparent">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              <h2 className="text-lg font-semibold">{t('pages.plans.selectedPlans')}</h2>
              <p className="text-xs text-muted-foreground">
                {cartItemCount === 0
                  ? t('pages.plans.noItemsSelected')
                  : t('pages.plans.itemsSelected', { count: cartItemCount })}
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="p-4">
            {cart.length === 0 ? (
              <div className="text-center text-muted-foreground py-12">
                ...
                <p className="text-sm font-medium">{t('pages.plans.noPlansSelected')}</p>
                <p className="text-xs mt-1 text-muted-foreground/70">
                  {t('pages.plans.clickToAdd')}
                </p>
              </div>
            ) : (
          ...
          <div className="p-4 border-t bg-gradient-to-t from-muted/30 to-transparent space-y-4">
            <div className="flex items-center justify-between">
              <span className="text-sm text-muted-foreground">
                {hasSubscriptionChanges ? t('pages.plans.newPrice') : t('pages.plans.monthlyTotal')}
              </span>
            ...
            {hasSubscriptionChanges && (
              ...
              >
                ...
                {cart.some(i => i.isUpgrade) ? t('pages.plans.confirmUpgrade') : t('pages.plans.confirmDowngrade')}
              </Button>
            )}
            ...
            {hasNewItems && (
              <Button className="w-full" size="lg" onClick={handleCheckout}>
                {t('pages.plans.proceedToCheckout')}
              </Button>
            )}
